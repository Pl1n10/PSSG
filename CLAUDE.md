...
- **No npm dependencies**: Everything is hand-rolled (JWT, Google auth, SendGrid calls). This keeps the Worker tiny and avoids bundling issues.
- **Module structure**:
  ...
  - `email.js` — SendGrid admin notification + user confirmation emails (`sendNotification()` + `sendConfirmation()`)
...
| `SENDGRID_API_KEY` | No* | SendGrid API key (*email won't send without it) |
| `EMAIL_FROM` | No* | Verified sender email in SendGrid |
...
Both are non-blocking (run in `ctx.waitUntil`). If SendGrid is not configured, both skip silently.
